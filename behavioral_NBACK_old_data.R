# Script to calculate dprime for the nback task from data from the 1.5T GE scanner

# This version is a function version to be used with the Shiny App BehAppy


behavioral_NBACK_old_data <- function(outputdir, logdir, subjects, outfile) {
  ...
  correct <- c(correct, 0)
  conditions <- c(conditions, "baseline")
  ...
  for (j in 1:length(ids)){
    ...
    fileName=paste(logdir, "\\", ids[[j]], "_NB.txt", sep="")
    
    if (file.exists(fileName)) {
      
      sub <- readLines(fileName)
      n <- grep("Event", sub, useBytes = TRUE)
      
      sub <- read.table(fileName, header = TRUE, skip = n-1)
